...

{
	...
	*/
	...
	KxVec << 400, 400, 400, 400, 400, 400;
	Kx = KxVec.asDiagonal();

	//Cartesian damping matrix 6x6
	DxVec << 70, 70, 70, 70, 70, 70;
	Dx = DxVec.asDiagonal();

	//Weighting matrix 7x7
	K0Vec << 70, 70, 70, 70, 70, 70, 70;
	K0 = K0Vec.asDiagonal();
	...
	//allcurrentvel(ppmaccomm, dq_pmac_update);	//good, radian/s
	
	//Break off motor drivers:
	//breakofftorquemode(ppmaccomm, motor[4]);
	//sleep(1);	
	...
	if (MotorBreakOff != 0)
		{
			printf("Reading Vel failed. exit:\n");
	
		}
		//If program got this velocity -> continue to compute torque
		else
		{
			printf("Break off Joint [%d] in torque mode.\n", motor[5]);
		}T_limit[0]
	*/
	//breakofftorquemode(ppmaccomm, motor[5]);		
	...
	while (ros::ok())
	{
		...
		else
		{
			...
			Jacob(0,0) = - 1.0*d5*(sin(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + 1.0*cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) + 1.0*cos(q_pmac_update[3])*sin(q_pmac_update[0])*sin(q_pmac_update[1])) - 1.0*d7*(cos(q_pmac_update[5])*(sin(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + 1.0*cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) + 1.0*cos(q_pmac_update[3])*sin(q_pmac_update[0])*sin(q_pmac_update[1])) + sin(q_pmac_update[5])*(cos(q_pmac_update[4])*(cos(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + 1.0*cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) + sin(q_pmac_update[4])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2])))) - d3*sin(q_pmac_update[0])*sin(q_pmac_update[1]);
			Jacob(0,1) = 1.0*d5*(1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[3]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[2])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - 1.0*d7*(sin(q_pmac_update[5])*(cos(q_pmac_update[4])*(cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[3]) + 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[2])*sin(q_pmac_update[4])) - cos(q_pmac_update[5])*(1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[3]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[2])*sin(q_pmac_update[1])*sin(q_pmac_update[3]))) + d3*cos(q_pmac_update[0])*cos(q_pmac_update[1]);
			Jacob(0,2) = 1.0*d7*(sin(q_pmac_update[5])*(sin(q_pmac_update[4])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) - cos(q_pmac_update[3])*cos(q_pmac_update[4])*(cos(q_pmac_update[2])*sin(q_pmac_update[0]) + 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2]))) - cos(q_pmac_update[5])*sin(q_pmac_update[3])*(cos(q_pmac_update[2])*sin(q_pmac_update[0]) + 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2]))) - 1.0*d5*sin(q_pmac_update[3])*(cos(q_pmac_update[2])*sin(q_pmac_update[0]) + 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2]));
			Jacob(0,3) = - 1.0*d5*(cos(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) + 1.0*cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - 1.0*d7*(cos(q_pmac_update[5])*(cos(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) + 1.0*cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - cos(q_pmac_update[4])*sin(q_pmac_update[5])*(sin(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) - cos(q_pmac_update[0])*cos(q_pmac_update[3])*sin(q_pmac_update[1])));
			Jacob(0,4) = 1.0*d7*sin(q_pmac_update[5])*(sin(q_pmac_update[4])*(cos(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) + cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - cos(q_pmac_update[4])*(cos(q_pmac_update[2])*sin(q_pmac_update[0]) + cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2])));
			Jacob(0,5) = 1.0*d7*(sin(q_pmac_update[5])*(sin(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - cos(q_pmac_update[5])*(cos(q_pmac_update[4])*(cos(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) + cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) + sin(q_pmac_update[4])*(cos(q_pmac_update[2])*sin(q_pmac_update[0]) + cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2]))));
			Jacob(0,6) = 0;

			Jacob(1,0) = d3*cos(q_pmac_update[0])*sin(q_pmac_update[1]) - d5*(sin(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) - cos(q_pmac_update[0])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - d7*(sin(q_pmac_update[5])*(cos(q_pmac_update[4])*(cos(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) + 1.0*cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) + sin(q_pmac_update[4])*(cos(q_pmac_update[2])*sin(q_pmac_update[0]) + 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2]))) + cos(q_pmac_update[5])*(sin(q_pmac_update[3])*(sin(q_pmac_update[0])*sin(q_pmac_update[2]) - cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) - cos(q_pmac_update[0])*cos(q_pmac_update[3])*sin(q_pmac_update[1])));
			Jacob(1,1) = d5*(cos(q_pmac_update[1])*cos(q_pmac_update[3])*sin(q_pmac_update[0]) - cos(q_pmac_update[2])*sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) + d7*(cos(q_pmac_update[5])*(cos(q_pmac_update[1])*cos(q_pmac_update[3])*sin(q_pmac_update[0]) - cos(q_pmac_update[2])*sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - sin(q_pmac_update[5])*(cos(q_pmac_update[4])*(1.0*cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[3]) + cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[0])*sin(q_pmac_update[1])) - 1.0*sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[2])*sin(q_pmac_update[4]))) + d3*cos(q_pmac_update[1])*sin(q_pmac_update[0]);
			Jacob(1,2) = d5*sin(q_pmac_update[3])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2])) - d7*(sin(q_pmac_update[5])*(sin(q_pmac_update[4])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + 1.0*cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - cos(q_pmac_update[3])*cos(q_pmac_update[4])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2]))) - cos(q_pmac_update[5])*sin(q_pmac_update[3])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2])));
			Jacob(1,3) = d5*(cos(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) + d7*(cos(q_pmac_update[5])*(cos(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - cos(q_pmac_update[4])*sin(q_pmac_update[5])*(sin(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) + 1.0*cos(q_pmac_update[3])*sin(q_pmac_update[0])*sin(q_pmac_update[1])));
			Jacob(1,4) = -d7*sin(q_pmac_update[5])*(sin(q_pmac_update[4])*(cos(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - 1.0*sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - cos(q_pmac_update[4])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - 1.0*cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2])));
			Jacob(1,5) = d7*(cos(q_pmac_update[5])*(cos(q_pmac_update[4])*(cos(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - 1.0*sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) + sin(q_pmac_update[4])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - 1.0*cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2]))) - sin(q_pmac_update[5])*(sin(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) + cos(q_pmac_update[3])*sin(q_pmac_update[0])*sin(q_pmac_update[1])));
			Jacob(1,6) = 0;

			Jacob(2,0) = 0;
			Jacob(2,1) = 1.0*d7*(sin(q_pmac_update[5])*(cos(q_pmac_update[4])*(sin(q_pmac_update[1])*sin(q_pmac_update[3]) - cos(q_pmac_update[1])*cos(q_pmac_update[2])*cos(q_pmac_update[3])) + 1.0*cos(q_pmac_update[1])*sin(q_pmac_update[2])*sin(q_pmac_update[4])) - 1.0*cos(q_pmac_update[5])*(cos(q_pmac_update[3])*sin(q_pmac_update[1]) + 1.0*cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[3]))) - d3*sin(q_pmac_update[1]) - d5*(cos(q_pmac_update[3])*sin(q_pmac_update[1]) + 1.0*cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[3]));
			Jacob(2,2) = 1.0*d7*(sin(q_pmac_update[5])*(1.0*cos(q_pmac_update[2])*sin(q_pmac_update[1])*sin(q_pmac_update[4]) + cos(q_pmac_update[3])*cos(q_pmac_update[4])*sin(q_pmac_update[1])*sin(q_pmac_update[2])) + 1.0*cos(q_pmac_update[5])*sin(q_pmac_update[1])*sin(q_pmac_update[2])*sin(q_pmac_update[3])) + 1.0*d5*sin(q_pmac_update[1])*sin(q_pmac_update[2])*sin(q_pmac_update[3]);
			Jacob(2,3) = - d5*(cos(q_pmac_update[1])*sin(q_pmac_update[3]) + 1.0*cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - 1.0*d7*(1.0*cos(q_pmac_update[5])*(cos(q_pmac_update[1])*sin(q_pmac_update[3]) + 1.0*cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) + cos(q_pmac_update[4])*sin(q_pmac_update[5])*(cos(q_pmac_update[1])*cos(q_pmac_update[3]) - cos(q_pmac_update[2])*sin(q_pmac_update[1])*sin(q_pmac_update[3])));
			Jacob(2,4) = 1.0*d7*sin(q_pmac_update[5])*(sin(q_pmac_update[4])*(cos(q_pmac_update[1])*sin(q_pmac_update[3]) + cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) + 1.0*cos(q_pmac_update[4])*sin(q_pmac_update[1])*sin(q_pmac_update[2]));
			Jacob(2,5) = -1.0*d7*(cos(q_pmac_update[5])*(cos(q_pmac_update[4])*(cos(q_pmac_update[1])*sin(q_pmac_update[3]) + cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - 1.0*sin(q_pmac_update[1])*sin(q_pmac_update[2])*sin(q_pmac_update[4])) + 1.0*sin(q_pmac_update[5])*(cos(q_pmac_update[1])*cos(q_pmac_update[3]) - 1.0*cos(q_pmac_update[2])*sin(q_pmac_update[1])*sin(q_pmac_update[3])));
			Jacob(2,6) = 0;

			Jacob(3,0) = -1.0*sin(q_pmac_update[0]);
			Jacob(3,1) = cos(q_pmac_update[0])*sin(q_pmac_update[1]);
			Jacob(3,2) = - 1.0*cos(q_pmac_update[2])*sin(q_pmac_update[0]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2]);
			Jacob(3,3) = 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[3])*sin(q_pmac_update[1]) - 1.0*sin(q_pmac_update[3])*(1.0*sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2]));
			Jacob(3,4) = sin(q_pmac_update[4])*(1.0*cos(q_pmac_update[3])*(1.0*sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) + 1.0*cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - 1.0*cos(q_pmac_update[4])*(1.0*cos(q_pmac_update[2])*sin(q_pmac_update[0]) + 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2]));
			Jacob(3,5) = - 1.0*cos(q_pmac_update[5])*(1.0*sin(q_pmac_update[3])*(1.0*sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - 1.0*sin(q_pmac_update[5])*(1.0*sin(q_pmac_update[4])*(1.0*cos(q_pmac_update[2])*sin(q_pmac_update[0]) + 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2])) + 1.0*cos(q_pmac_update[4])*(1.0*cos(q_pmac_update[3])*(1.0*sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) + 1.0*cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])));
			Jacob(3,6) = - 1.0*cos(q_pmac_update[5])*(1.0*sin(q_pmac_update[3])*(1.0*sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - 1.0*sin(q_pmac_update[5])*(1.0*sin(q_pmac_update[4])*(1.0*cos(q_pmac_update[2])*sin(q_pmac_update[0]) + 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*sin(q_pmac_update[2])) + 1.0*cos(q_pmac_update[4])*(1.0*cos(q_pmac_update[3])*(1.0*sin(q_pmac_update[0])*sin(q_pmac_update[2]) - 1.0*cos(q_pmac_update[0])*cos(q_pmac_update[1])*cos(q_pmac_update[2])) + 1.0*cos(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])));


			Jacob(4,0) = cos(q_pmac_update[0]);
			Jacob(4,1) = sin(q_pmac_update[0])*sin(q_pmac_update[1]);
			Jacob(4,2) = cos(q_pmac_update[0])*cos(q_pmac_update[2]) - 1.0*cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2]);
			Jacob(4,3) = sin(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) + 1.0*cos(q_pmac_update[3])*sin(q_pmac_update[0])*sin(q_pmac_update[1]);
			Jacob(4,4) = cos(q_pmac_update[4])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - 1.0*cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2])) - 1.0*sin(q_pmac_update[4])*(cos(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - 1.0*sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3]));
			Jacob(4,5) = sin(q_pmac_update[5])*(cos(q_pmac_update[4])*(cos(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - 1.0*sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) + sin(q_pmac_update[4])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - 1.0*cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2]))) + cos(q_pmac_update[5])*(1.0*sin(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) + 1.0*cos(q_pmac_update[3])*sin(q_pmac_update[0])*sin(q_pmac_update[1]));
			Jacob(4,6) = sin(q_pmac_update[5])*(cos(q_pmac_update[4])*(cos(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) - 1.0*sin(q_pmac_update[0])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) + sin(q_pmac_update[4])*(cos(q_pmac_update[0])*cos(q_pmac_update[2]) - 1.0*cos(q_pmac_update[1])*sin(q_pmac_update[0])*sin(q_pmac_update[2]))) + cos(q_pmac_update[5])*(1.0*sin(q_pmac_update[3])*(cos(q_pmac_update[0])*sin(q_pmac_update[2]) + cos(q_pmac_update[1])*cos(q_pmac_update[2])*sin(q_pmac_update[0])) + 1.0*cos(q_pmac_update[3])*sin(q_pmac_update[0])*sin(q_pmac_update[1]));

			Jacob(5,0) = 0;
			Jacob(5,1) = 1.0*cos(q_pmac_update[1]);
			Jacob(5,2) = 1.0*sin(q_pmac_update[1])*sin(q_pmac_update[2]);
			Jacob(5,3) = 1.0*cos(q_pmac_update[1])*cos(q_pmac_update[3]) - 1.0*cos(q_pmac_update[2])*sin(q_pmac_update[1])*sin(q_pmac_update[3]);
			Jacob(5,4) = sin(q_pmac_update[4])*(1.0*cos(q_pmac_update[1])*sin(q_pmac_update[3]) + 1.0*cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) + 1.0*cos(q_pmac_update[4])*sin(q_pmac_update[1])*sin(q_pmac_update[2]);
			Jacob(5,5) = cos(q_pmac_update[5])*(1.0*cos(q_pmac_update[1])*cos(q_pmac_update[3]) - 1.0*cos(q_pmac_update[2])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - 1.0*sin(q_pmac_update[5])*(1.0*cos(q_pmac_update[4])*(1.0*cos(q_pmac_update[1])*sin(q_pmac_update[3]) + 1.0*cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - 1.0*sin(q_pmac_update[1])*sin(q_pmac_update[2])*sin(q_pmac_update[4]));
			Jacob(5,6) = cos(q_pmac_update[5])*(1.0*cos(q_pmac_update[1])*cos(q_pmac_update[3]) - 1.0*cos(q_pmac_update[2])*sin(q_pmac_update[1])*sin(q_pmac_update[3])) - 1.0*sin(q_pmac_update[5])*(1.0*cos(q_pmac_update[4])*(1.0*cos(q_pmac_update[1])*sin(q_pmac_update[3]) + 1.0*cos(q_pmac_update[2])*cos(q_pmac_update[3])*sin(q_pmac_update[1])) - 1.0*sin(q_pmac_update[1])*sin(q_pmac_update[2])*sin(q_pmac_update[4]));
			...
			for (int i=0; i<6; i++)
			{
				...
				printf("Joint [%d]:T_PD [%.2f] T_FR [%.2f] T_GR [%.2f] \n", i+1, (Tau_stif(i,0)*1000)/(Nhd[i]*Kt[i]*Icon[i]), Friction_I(i,0)*1000/Icon[i], (Gravity_T(i,0)*1000)/(Nhd[i]*Kt[i]*Icon[i]));
			}

			/*
			//PD torque computation and send torque command
			int TorqIsSet[7], AllTorqueSet;
			AllTorqueSet = 0;
			for (int i=3; i<4; i++)
			{
				Tau_stif(i,0) = Kp[i]*theta_err1(i,0) + Kd[i]*dtheta_err(i,0);
				//Command torque
				//Tau_stif(i,0) = 0;
				Tau_I(i,0) = ((Tau_stif(i,0) + Gravity_T(i,0))*1000)/(Nhd[i]*Kt[i]*Icon[i]) + Friction_I(i,0)*1000/Icon[i];	//joint i
				//Joint torque limits
