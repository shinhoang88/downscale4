...
namespace gazebo
{   
  class ROSdownscalePlugin : public ModelPlugin
  {

//    public: ROSdownscalePlugin()
//    {
//
//      // Start up ROS
...
//////////////////////sub for position and torque of quadruped/////////////////////////////////////////
 this->subp = this->node->subscribe<downscale4::seven>("downscale_jointp", 100, &ROSdownscalePlugin::ROSCallbackp, this );

 this->subt = this->node->subscribe<downscale4::seven>("downscale_jointt", 100, &ROSdownscalePlugin::ROSCallbackt, this );
...
////////////////////////////////////////////////////////////////////////////
      ...
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          boost::bind(&ROSdownscalePlugin::OnUpdate, this));
    }
	...
	*/
	// New gazebo
	actp.a = this->joint1_->Position(1);
	actp.b = this->joint2_->Position(1);
	actp.c = this->joint3_->Position(1);
	actp.d = this->joint4_->Position(1);
	actp.e = this->joint5_->Position(1);
	actp.f = this->joint6_->Position(1);
	actp.g = this->joint7_->Position(1);
...
// get joint torque 
	...
	*/
	//New gazebo
	ignition::math::Vector3d j1torque = joint1ft.body2Force;
	ignition::math::Vector3d j2torque = joint2ft.body2Force;
	ignition::math::Vector3d j3torque = joint3ft.body2Force;
	ignition::math::Vector3d j4torque = joint4ft.body2Force;
	ignition::math::Vector3d j5torque = joint5ft.body2Force;
	ignition::math::Vector3d j6torque = joint6ft.body2Force;
	ignition::math::Vector3d j7torque = joint7ft.body2Force;
...
//	actt.g = j7torque.z;
	...
	*/
	// New Gazebo
	actt.a = j7torque[0]; // x torque sensing // able to modify specific joint torque
       	actt.b = j7torque[1];
       	actt.c = j7torque[2];
       	actt.d = j3torque[0];
       	actt.e = j3torque[1];
...
// Pointer to the joint controller // tempamlate need to study! // innertia. coliolis 알아보기
 ...
 ros::Subscriber subt;
  ...
  GZ_REGISTER_MODEL_PLUGIN(ROSdownscalePlugin)
}
